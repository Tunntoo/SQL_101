--1
with CTE as (SELECT company_id, title, description, count(job_id) as so_luong
...
--2
with app as(select category, product, sum(spend) as total_spend
from product_spend
...
elec as (select category, product, sum(spend) as total_spend
from product_spend
...
--4
with cte as (select p.page_id, plike.user_id, plike.page_id as like_id
from pages as p
...
order by page_id
--5
with july as(select distinct user_id, extract(month from event_date) as month
...
june as (select distinct user_id, extract(month from event_date) as month
...
group by july.month
--6
with cte as (select trans_date, country, id,
state,
